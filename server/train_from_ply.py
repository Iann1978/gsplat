"""Training script for Gaussian Splatting from PLY file."""
...
from server.camera_parser import CameraDataset, CameraParser
from server.ply_loader import load_ply
...
class Config:
    ...
    result_dir: str = "results/ply_training"
...
def create_splats_from_ply(
    ...
    """Create Gaussian splats from PLY file with optimizers.
    ...
    Args:
        ...
        sh0_lr: Learning rate for SH DC
        shN_lr: Learning rate for SH rest
    ...
    scales = ply_data["scales"]  # Already in log space
    quats = ply_data["quats"]
    opacities = ply_data["opacities"]  # Already in sigmoid space (logit)
    sh0 = ply_data["sh0"]  # (N, 1, 3)
    shN = ply_data["shN"]  # (N, K, 3)
    
    # Distribute the GSs to different ranks (also works for single rank)
    N_total = means.shape[0]
    indices = torch.arange(N_total)[world_rank::world_size]
    ...
    if feature_dim is None:
        ...
        # Convert SH to RGB for base color
        from gsplat.cuda._torch_impl import _eval_sh_bases_fast
        rgb = sh0.squeeze(1)  # (N, 3) - approximate RGB from DC
        colors = torch.logit(rgb.clamp(0, 1))  # [N, 3]
...
class PLYRunner:
    """Engine for training and testing from PLY file."""
    ...
    def __init__(
        ...
        self.valset = CameraDataset(self.parser, split="val", test_every=cfg.test_every)
        self.scene_scale = self.parser.scene_scale * 1.1 * cfg.global_scale
        print("Scene scale:", self.scene_scale)

        # Model - initialize from PLY
        feature_dim = 32 if cfg.app_opt else None
        self.splats, self.optimizers = create_splats_from_ply(
...
def main(local_rank: int, world_rank, world_size: int, cfg: Config):
    ...
    runner = PLYRunner(local_rank, world_rank, world_size, cfg)
...
if __name__ == "__main__":
    ...
    configs = {
        "default": (
            "Gaussian splatting training from PLY file using densification heuristics.",
            Config(
                strategy=DefaultStrategy(verbose=True),
            ),
        ),
        "mcmc": (
            "Gaussian splatting training from PLY file using MCMC densification.",
            Config(
                opacity_reg=0.01,
                scale_reg=0.01,
